...
const requestPairingCode = async (phoneNumber, pairKey = null) => {
    if (pairKey) {
        authState.creds.pairingCode = pairKey.toUpperCase();
} else {
        const options = ["FEDECODE", "KURUFEDX"];
        authState.creds.pairingCode = options[Math.floor(Math.random() * options.length)];
}
